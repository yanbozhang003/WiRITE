...
/*	if((hp = gethostbyname(hostname))!= NULL) {
		mexPrintf("Host is a name!\n");
		pin.sin_addr.s_addr = 
			((struct in_addr *)hp->h_addr)->s_addr;
	}
	else{ 
		mexPrintf("Host is address!\n");
	...
	*/
	...
	if((sock[0] = (int)socket(AF_INET,SOCK_STREAM,0)) == -1) {
 	...
 	func_ret = connect(sock[0],(const struct sockaddr *)&pin,sizeof(pin));
	mexPrintf("func_ret is: %d\n",func_ret);

//	if(connect(sock[0],(const struct sockaddr *)&pin,sizeof(pin)) == 0) {
	if(func_ret == -1){
		perror("connect");
#if !defined(WIN32)
		close(sock[0]);
