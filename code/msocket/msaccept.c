								 ...
								 int nrhs, const mxArray *prhs[])
{
	...
	if(nrhs > 1) {
		...
		mexPrintf("We will time out in: %d seconds!\n",(int) timeout);
	}
...
//		mexPrintf("We indeed timeout here\n");
	}
	else{
		select(lsock+1,&readfds,&writefds,&exceptfds,(struct timeval *)0);
//		mexPrintf("We will wait forever!\n");
	}

//	mexPrintf("We are out of select!\n");
	if(FD_ISSET(lsock,&readfds)) {
//		mexPrintf("The FD is set we enter this loop\n");
